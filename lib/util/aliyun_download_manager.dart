...
// import 'package:flutter_testaliplayer/model/custom_downloader_model.dart';
// import 'package:flutter_testaliplayer/util/database_utils.dart';
// import 'package:path_provider/path_provider.dart';

// class AliyunDownloadManager {
//   static AliyunDownloadManager _instance = AliyunDownloadManager._privateConstructor();
//   FlutterAliDownloader _flutterAliDownloader;
//   DBUtils _dbUtils;
//   //下载保存地址
//   String _downloadSavePath;
//   //prepared集合
//   List<CustomDownloaderModel> _preparedList = [];
...
//     return src != null && dst != null && src.videoId == dst.videoId && src.vodDefinition == dst.vodDefinition;
//   }
...
//         CustomDownloaderModel customDownloaderModel = CustomDownloaderModel.fromJson(element);
...
//         .prepare(map[DataSourceRelated.TYPE_KEY], map[DataSourceRelated.VID_KEY],
...
//         return Future.error('${customDownloaderModel.videoId} , ${customDownloaderModel.vodDefinition}  has added');
//       }
//     });
//     var json = customDownloaderModel.toJson();
//     _preparedList.add(customDownloaderModel);
//     _flutterAliDownloader.selectItem(customDownloaderModel.videoId, customDownloaderModel.index);
...
//     String key = customDownloaderModel.videoId + '_' + customDownloaderModel.index.toString();
...
//     _controller.addStream(_flutterAliDownloader.start(customDownloaderModel.videoId, customDownloaderModel.index));
...
//           customDownloaderModel.stateMsg = event[EventChanneldef.DOWNLOAD_PROGRESS] + "%";
//           _sink.add(customDownloaderModel);
//         } else if (event[EventChanneldef.TYPE_KEY] == EventChanneldef.DOWNLOAD_PROCESS && customDownloaderModel.videoId == event['vid'] && customDownloaderModel.index == event['index']) {
//           customDownloaderModel.stateMsg = "ProcessingProgress \n" + event[EventChanneldef.DOWNLOAD_PROGRESS] + "%";
//           _sink.add(customDownloaderModel);
//         } else if (event[EventChanneldef.TYPE_KEY] == EventChanneldef.DOWNLOAD_COMPLETION && customDownloaderModel.videoId == event['vid'] && customDownloaderModel.index == event['index']) {
...
//         } else if (event[EventChanneldef.TYPE_KEY] == EventChanneldef.DOWNLOAD_ERROR && customDownloaderModel.videoId == event['vid'] && customDownloaderModel.index == event['index']) {
//           customDownloaderModel.downloadState = DownloadState.ERROR;
//           print("aliyun : download_error --- ${event['errorCode']}  ------   ${event['errorMsg']}");
...
//     if (customDownloaderModel.downloadState == DownloadState.START || customDownloaderModel.downloadState == DownloadState.PREPARE) {
//       _flutterAliDownloader.stop(customDownloaderModel.videoId, customDownloaderModel.index);
//       customDownloaderModel.downloadState = DownloadState.STOP;
...
//     _dbUtils.delete(map);
//     _flutterAliDownloader.delete(customDownloaderModel.videoId, customDownloaderModel.index);
//     _flutterAliDownloader.release(customDownloaderModel.videoId, customDownloaderModel.index);
//     String key = customDownloaderModel.videoId + '_' + customDownloaderModel.index.toString();
//     _controllerMap.remove(key);
//     return Future.value(customDownloaderModel);
//   }
