...
import 'package:flutter_testaliplayer/model/video_model.dart';
    ...
    if (_dataList != null && _dataList.length > 0 && _curIdx < _dataList.length) {
      ...
      if (widget.playMode == ModeType.URL) {
        ...
        NetWorkUtils.instance.getHttp(HttpConstant.GET_STS, successCallback: (data) {
          fAliListPlayer?.moveTo(uid: model.videoId, accId: data["accessKeyId"], accKey: data["accessKeySecret"], token: data["securityToken"], region: DataSourceRelated.DEFAULT_REGION);
          print('========${model.videoId}');
        }, errorCallback: (error) {
          print("error");
        });
