...
import 'package:flutter_testaliplayer/page/player_fragment/cache_config_fragment.dart';
import 'package:flutter_testaliplayer/page/player_fragment/options_fragment.dart';
import 'package:flutter_testaliplayer/page/player_fragment/play_config_fragment.dart';
import 'package:flutter_testaliplayer/page/player_fragment/track_fragment.dart';
import 'package:flutter_testaliplayer/util/formatter_utils.dart';
import 'package:flutter_testaliplayer/widget/aliyun_slider.dart';
...
class _PlayerPageState extends State<PlayerPage> with WidgetsBindingObserver {
  ...
  _initListener() {
    ...
    fAliplayer?.setOnTrackChanged((value, playerId) {
      ...
      if (info != null && info.trackDefinition?.length != null) {
            ...
            InkWell(
                ...
                onTap: () {
                  if (Platform.isIOS) {
                    ...
                    fAliplayer?.snapshot(_snapShotPath! + "/snapshot_" + new DateTime.now().millisecondsSinceEpoch.toString() + ".png");
                  }
                }),
          ],
        ),
    ...
    if (_inSeek && _thumbnailSuccess) {
      return Container(
        ...
        child: Wrap(
          ...
          children: [
            Text("${FormatterUtils.getTimeformatByMs(_currentPosition)}", style: TextStyle(color: Colors.white), textAlign: TextAlign.center),
            _imageProvider == null
                ? Container()
                : Image(
                    width: width / 2,
    ...
    if (_showTipsWidget) {
      return Container(
        ...
        child: Column(
          ...
          children: [
            Text(_tipsContent!, maxLines: 3, style: TextStyle(color: Colors.red), textAlign: TextAlign.center),
            SizedBox(
              height: 5.0,
            ),
            OutlineButton(
    ...
    return Offstage(
      ...
      child: Container(
        ...
        child: Wrap(
          ...
          crossAxisAlignment: WrapCrossAlignment.center,
          children: [
            Text("当前为移动网络", style: TextStyle(color: Colors.white), textAlign: TextAlign.center),
            SizedBox(
              height: 30.0,
            ),
            Wrap(
    ...
    if (orientation == Orientation.portrait) {
      return BottomNavigationBar(
        ...
        items: [
          ...
          BottomNavigationBarItem(label: ('play_cfg'), icon: Icon(Icons.control_point)),
          BottomNavigationBarItem(label: ('cache_cfg'), icon: Icon(Icons.control_point)),
